//#define CONTROL_PISTON_TEST
#ifdef CONTROL_PISTON_TEST
...
void initSystem(){
    ...
    initPid(&piston_press_control, 0, 0, 0, 0, 0, 0);
}
...
void EVAL(timerInterrupt, TIMER_SENSORS){
    ...
    sprintf(buffer, "%.1lf %.1lf %.1lf %.1lf %.1lf %.1lf %llu %.2lf\n", \
                        piston_press.val, \
                        piston_press_control.kp, \
                        piston_press_control.ki, \
                        piston_press_control.kd, \
                        piston_press_control.isat,\
                        piston_press_control.osat,\
                        est_time - time_zero,\
                        sob_imp);
...
int main(void){
    initSystem();
    int sp = 0;
    resetPid(&piston_press_control, time);
    while(true){
        if(serialAvailable()){
            ...
            if(op == 0){
                ...
                moveHome(&my_piston); 
                piston_press_control.kp = kp_aux;
                piston_press_control.kd = kd_aux;
                piston_press_control.ki = ki_aux;
                piston_press_control.isat = isat_aux;
                piston_press_control.osat = osat_aux;
                resetPid(&piston_press_control, time); 
                sp = 0;
	...
	//
        control = computePid(&piston_press_control, error, time);
