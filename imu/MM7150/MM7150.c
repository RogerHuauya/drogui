...
void gets_I2C1(UINT8 *ucRdptr, UINT16 usLength, BOOL bAdjust) {
  ...
  if (bAdjust){
    Wire.sendRequest(SLAVE_ADDR, 40, I2C_STOP);  // NON-blocking read (request 256 bytes)
    Wire.finish();

    int i = 0;
    while(Wire.available()){
      ucRdptr[i++] = Wire.readByte();
    }

    /*
    Serial.println("kha");
    Wire.requestFrom(SLAVE_ADDR, 2); // Read from Slave (string len unknown, request full buffer)
    // Check if error occured
    
    if(Wire.getError()) Serial.print("FAIL\n");
    else Wire.read(ucRdptr, Wire.available());
    usLength = ((ucRdptr[1] << BYTE_SHIFT) | ucRdptr[0]) - 2; // Actual length is value of first 2 bytes minus 2 (because we read 2 already)
    Serial.print("aiuda ctv ");
    Serial.println(usLength + 2);
    ucSize = usLength;
    Wire.requestFrom(SLAVE_ADDR, UINT8(usLength)) ; // Read from Slave (string len unknown, request full buffer)
    // Check if error occured
    
    if(Wire.getError()) Serial.print("FAIL\n");
    else Wire.read(ucRdptr + 2, Wire.available());*/

  }
  else
  {
    //ta bien
    Wire.sendRequest(SLAVE_ADDR, usLength+5, I2C_STOP);  // NON-blocking read (request 256 bytes)
    Wire.finish();
...
UINT8 hid_i2c_cmd_process(UINT8 *ucCmdDatbuf, UINT8 ucCmd_req, UINT8 ucReport_id)
{
    ...
    switch(ucCmd_req)
    {
        ...
        case HID_GET_RPT_FEAT:
            ...
            Serial.println("chichico se la come : 1");
            if ( HOST_SF_LIB_HID_Get_Report(GET_RPT_FEAT, ucCmdDatbuf, 0) )
                return HID_GET_RPT_FEAT_FAIL;                       // command failed
            Serial.println("chichico se la come : 2");
            ...
            Serial.println("chichico se la come : 3");                  // invalid reportID
