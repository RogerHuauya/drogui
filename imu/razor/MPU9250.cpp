...
MPU9250::MPU9250( uint8_t address, uint32_t clock_frequency )
{
	_I2Caddr = address;
...
uint8_t MPU9250::writeByteWire(uint8_t deviceAddress, uint8_t registerAddress,
                            uint8_t data)
{
	Wire.setClock(_interfaceSpeed);			// Reset to the desired speed, in case other devices required a slowdown
