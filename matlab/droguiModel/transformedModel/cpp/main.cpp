...
int main(){
	...
	for(double tim = dt ; tim < 10; tim += dt){
		...
		u[0] = 0.1*( (12*0.1*cos(tim) - 12*noise(drogui.ap[0]) + 80*(sp[0] - noise(drogui.a[0]))) -  + 0.77*(drogui.ap[2]*drogui.ap[1]));
		u[1] = 0.1*( (-12*noise(drogui.ap[1]) + 80*(sp[1] - noise(drogui.a[1]))) -  - 0.78*(drogui.ap[0]*drogui.ap[2]));
		u[2] = 0.1724*( (-50*noise(drogui.ap[2]) + 100*(sp[2] - noise(drogui.a[2]))) -  + 0.011*(drogui.ap[1]*drogui.a[0]));
		u[3] = 1.0/(0.29*cos(drogui.a[0])*cos(drogui.a[1]))*( (-50*noise(drogui.rp[2], 0.01) + 100*(sp[3] - noise(drogui.r[2], 0.05))) );
