...
void xyzControlTask(){
    ...
    if(state == CONTROL_LOOP){
        ...
        vx_ref = x_ref;//computePid(&x_control, x_ref - x, TIME, 0);
        vy_ref = y_ref;//computePid(&y_control, y_ref - y, TIME, 0);
...
void initControlTasks(){
    ...
    initPid(&xp_control,  0.01, 0, 0, 0, 50 , 5, ANG_MAX, D_SG);
    initPid(&yp_control,  0.01, 0, 0, 0, 50 , 5, ANG_MAX, D_SG);
