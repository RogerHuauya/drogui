...
uint8_t defaultCfgRate[6]  = { 0x64, 0, 1, 0, 1, 0 };
uint8_t defaultCfgMsg[3]   = {UBX_CLASS_NAV, UBX_NAV_PVT, 1};
//uint8_t defaultCfgGnss[3] = {0, 0, 0, 0, GPS, 1,}
uint8_t defaultCfgRst[4] = { 0x00, 0x00, 0x09, 0x00};
...
void cfgRst(ubxPacket *mp, uint8_t *cfgRstArray){

	mp->cls = UBX_CLASS_CFG;
	mp->id = UBX_CFG_RST;
	mp->len = 6;
	for( int i = 0; i < 6; i++ ) mp->payload[i] = cfgRstArray[i];
